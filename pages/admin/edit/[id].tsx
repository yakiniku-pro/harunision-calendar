...
import { doc, getDoc, updateDoc, query, getDocs, orderBy, serverTimestamp, collection, where, deleteField } from "firebase/firestore";
...
export default function EditEventAdmin() {
  ...
  useEffect(() => {
    ...
    try {
      // ★ groupsに加えて、allPersons と allMemberships もここで取得する
      const groupsSnap = await getDocs(query(collection(db, "groups"), orderBy("name")));
...
useEffect(() => {
  if (typeof id !== 'string' || !id || allPersons.length === 0) return;
  ...
  const fetchEvent = async () => {
    ...
    setTicketSales(data.ticketSales?.map((s:any) => ({...s, startAt: formatDateTime(s.startAt), endAt: formatDateTime(s.endAt)})) || [{ saleName: '', startAt: '', endAt: '', url: '' }]);
    ...
    // ★★★ ここからが修正箇所 ★★★
    // グループに所属するメンバーを特定
    const activeMembershipPids = allMemberships.filter(m => m.groupId === data.groupId && m.leftAt === null).map(m => m.personId);
    ...
    // 保存されているparticipatingMemberIdsを元にチェックボックスの初期状態を設定
    const initialChecklist: { [id: string]: boolean } = {};
    if (data.participatingMemberIds) {
      regularMembers.forEach(member => {
        initialChecklist[member.id] = data.participatingMemberIds.includes(member.id);
      });
      if (groupPerson) {
        setIsGroupChekiAvailable(data.participatingMemberIds.includes(groupPerson.id));
      }
    } else { // 古いデータ用のフォールバック
      regularMembers.forEach(member => { initialChecklist[member.id] = true; });
  ...
  fetchEvent();
}, [id, router, allPersons, allMemberships]);
...
const handleSubmit = async () => {
  ...
  try {
    const participatingMemberIds = groupMembers.filter(member => memberChecklist[member.id]).map(member => member.id);
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <FormSection title="参加メンバー">
        {groupId ? (
          ...
          <p className="text-sm text-gray-500">（グループ情報読み込み中...）</p>
        )}
      </FormSection>
