      ...
      if (currentUser && ADMIN_UIDS.includes(currentUser.uid)) {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            // ★★★ エラー修正箇所 ★★★
            const toDateTimeLocalString = (timestamp: any) => timestamp ? format(timestamp.toDate(), "yyyy-MM-dd'T'HH:mm") : '';
            let salesData: SalePeriod[] = [];
            // データが新しい配列形式か、古いオブジェクト形式かを判定
            if (Array.isArray(event.ticketSales)) {
                salesData = event.ticketSales.map((s: any) => ({ ...s, startAt: toDateTimeLocalString(s.startAt), endAt: toDateTimeLocalString(s.endAt) }));
            } else if (typeof event.ticketSales === 'object' && event.ticketSales !== null) {
                // 古いオブジェクト形式の場合、新しい配列形式に変換
                salesData = [
                    { saleName: '最速先行', ...event.ticketSales.preSaleFastest, startAt: toDateTimeLocalString(event.ticketSales.preSaleFastest?.startAt), endAt: toDateTimeLocalString(event.ticketSales.preSaleFastest?.endAt) },
                    { saleName: '先行販売', ...event.ticketSales.preSaleGeneral, startAt: toDateTimeLocalString(event.ticketSales.preSaleGeneral?.startAt), endAt: toDateTimeLocalString(event.ticketSales.preSaleGeneral?.endAt) },
                    { saleName: '一般販売', ...event.ticketSales.generalSale,    startAt: toDateTimeLocalString(event.ticketSales.generalSale?.startAt),    endAt: toDateTimeLocalString(event.ticketSales.generalSale?.endAt) },
                ].filter(s => s.startAt || s.endAt || s.url); // データがあるものだけを抽出
            }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const updatedEventData: { [key: string]: any } = {
        ...
        ticketSales: ticketSales.filter(s => s.saleName).map(s => ({ ...s, startAt: toTimestamp(s.startAt), endAt: toTimestamp(s.endAt) })),
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <FormSection title="詳細情報">
        ...
        <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-gray-700">静止画撮影</label><select value={photoPolicy.still} onChange={e => setPhotoPolicy({...photoPolicy, still: e.target.value})} className="mt-1 w-full border p-2 rounded-md bg-white"><option value="不明">不明</option><option value="OK">OK</option><option value="NG">NG</option></select></div><div><label className="block text-sm font-medium text-gray-700">動画撮影</label><select value={photoPolicy.video} onChange={e => setPhotoPolicy({...photoPolicy, video: e.target.value})} className="mt-1 w-full border p-2 rounded-md bg-white"><option value="不明">不明</option><option value="OK">OK</option><option value="NG">NG</option></select></div></div>
      ...
      <FormSection title="画像情報">
        <div><h3 className="text-md font-semibold text-gray-800">イベント写真</h3>{currentEventPhotoUrl && <div className="my-2"><Image src={currentEventPhotoUrl} alt="現在のイベント写真" width={320} height={180} className="rounded" /><button onClick={() => handleDeleteImage('event')} className="mt-2 text-sm text-red-600 hover:underline">この写真を削除</button></div>}<label className="block text-sm font-medium text-gray-700">{currentEventPhotoUrl ? '新しいイベント写真に差し替え' : 'イベント写真をアップロード'}</label><input type="file" accept="image/*" onChange={e => setEventImage(e.target.files?.[0] || null)} className="mt-1 block w-full text-sm"/></div>
        <hr className="my-4"/>
        <div><h3 className="text-md font-semibold text-gray-800">メンバー写真</h3>{currentMemberPhotoUrl && <div className="my-2"><Image src={currentMemberPhotoUrl} alt="現在のメンバー写真" width={320} height={180} className="rounded" /><button onClick={() => handleDeleteImage('member')} className="mt-2 text-sm text-red-600 hover:underline">この写真を削除</button></div>}<label className="block text-sm font-medium text-gray-700">{currentMemberPhotoUrl ? '新しいメンバー写真に差し替え' : 'メンバー写真をアップロード'}</label><input type="file" accept="image/*" onChange={e => setMemberImage(e.target.files?.[0] || null)} className="mt-1 block w-full text-sm"/></div>
      </FormSection>
